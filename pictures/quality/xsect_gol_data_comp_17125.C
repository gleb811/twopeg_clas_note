void xsect_gol_data_comp_17125(){


string file_names_m12[8];
string file_names_m23[8];
string file_names_th[8];
string file_names_alph[8];

Float_t integral;
Float_t arr_m12_val[8][16];
Float_t arr_m12_xsect[8][16];
Float_t arr_m12_err[8][16];

Float_t arr_m23_val[8][16];
Float_t arr_m23_xsect[8][16];
Float_t arr_m23_err[8][16];

Float_t arr_th_val[8][14];
Float_t arr_th_xsect[8][14];
Float_t arr_th_err[8][14];

Float_t arr_alph_val[8][14];
Float_t arr_alph_xsect[8][14];
Float_t arr_alph_err[8][14];
...
TFile *MyFile2 = new TFile("out_19Oct_0_0003_17125.root","READ");
...
for (Short_t j=0; j<=7; j++){
W[j] = 1.6125+0.025*j;
qqq << "h_odn_inv_m12_" << 10000*(1.6125+0.025*j);
cout << qqq.str().c_str() <<"\n";
gDirectory->GetObject(qqq.str().c_str(),h_m12[j]);
qqq.str("");

qqq << "h_odn_inv_m23_" << 10000*(1.6125+0.025*j);
cout << qqq.str().c_str() <<"\n";
gDirectory->GetObject(qqq.str().c_str(),h_m23[j]);
qqq.str("");

qqq << "h_odn_theta_2_" << 10000*(1.6125+0.025*j);
cout << qqq.str().c_str() <<"\n";
gDirectory->GetObject(qqq.str().c_str(),h_th[j]);
qqq.str("");

qqq << "h_odn_alpha_" << 10000*(1.6125+0.025*j);
cout << qqq.str().c_str() <<"\n";
...
integral = integral +arr_alph_xsect_1dim[i] ;
...
newpad = new TPad("newpad","a transparent pad",0.,0.,1.,1.);
 ...
 TLatex tex;
 qqq.str("");
qqq << "Q^{2} =  0.0005 GeV^{2}, W = 1.7125 GeV";
